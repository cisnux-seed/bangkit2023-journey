...
class SeatsView : View {
    ...
    private fun booking(position: Int) {
        for (seat in seats) {
            // multiple selection
//            if (!seat.isBooked) seat.isBooked = false
            // single selection
            seat.isBooked = false
        }
        val onSelectedSeat = seats[position].apply {
            isBooked = true
        }
        seat = onSelectedSeat
        /**
         * Kita menggunakan fungsi invalidate untuk me-refresh
         * metode onDraw dalam SeatsView. Dengan begitu, posisi
         * kursi yang dipilih (booking) sudah disimpan dalam array
         * seats dan SeatsView siap digunakan.
         * */
        invalidate()
    }
