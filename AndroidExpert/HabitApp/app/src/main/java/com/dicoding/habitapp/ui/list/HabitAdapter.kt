...
class HabitAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): HabitViewHolder {
        ...
        return HabitViewHolder(itemView = view)
    }

    override fun onBindViewHolder(holder: HabitViewHolder, position: Int) {
        //TODO 9 : Get data and bind them to ViewHolder
        val habit = getItem(position)
        habit?.let(block = holder::bind)
    }
        ...
        fun bind(habit: Habit) {
            ...
            ivPriority.setImageDrawable(
                when (habit.priorityLevel) {
                    context.getString(R.string.high) -> AppCompatResources.getDrawable(
                        context,
                        R.drawable.ic_priority_high
                    )
                    context.getString(R.string.medium) -> AppCompatResources.getDrawable(
                        context,
                        R.drawable.ic_priority_medium
                    )
                    else -> AppCompatResources.getDrawable(context, R.drawable.ic_priority_low)
                }
            )
