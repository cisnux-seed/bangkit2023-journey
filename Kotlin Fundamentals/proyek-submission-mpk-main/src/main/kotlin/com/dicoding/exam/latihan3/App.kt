 ...
 *
 */
fun <T> checkType(args: T): String = "Yes! it's ${
    when (args) {
        is Int -> "Integer"
        is String -> "String"
        is Boolean -> "Boolean"
        is Double -> "Double"
        is List<*> -> if (args.isNotEmpty()) "List<${args.first()?.javaClass?.simpleName}>" else "List"
        is Map<*, *> -> if (args.isNotEmpty()) "Map<${args.keys.first()?.javaClass?.simpleName},${args.values.first()?.javaClass?.simpleName}>" else "Map"
        else -> "Any"
    }
}"
